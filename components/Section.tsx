...
export default function Section({
  ...
  return (
    <section
      id={id}
      className={`scroll-mt-24 border-t border-line px-6 py-12 md:px-10 md:py-16 ${
      ...
      <div className={`mx-auto ${width === "wide" ? "max-w-5xl" : "max-w-4xl"}`}>
        <Reveal>
          <header className="mb-8 md:mb-10">
            ...
            <h2 className={`${displayClass} text-[clamp(1.6rem,3.4vw,2.5rem)] text-ink`}>
              {heading}
            </h2>
            {lead && (
              <p className="jp-wrap mt-3 max-w-2xl text-sm leading-relaxed text-ink-soft">
                {lead}
              </p>
            )}
          </header>
